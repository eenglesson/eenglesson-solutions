    ...
    <>
      ...
      >
        <aside className={`flex justify-center z-40 h-full gap-2 ${textColor}`}>
          <div className={`flex ${borderColor} border-r-2 pr-2`}>
        ...
        >
          <span
            className={`${bgColor} h-[2px] rounded-full transition-transform duration-300 ${
              hasClicked
                ? isOpen
                  ? 'animate-line1Open'
                  : 'animate-line1Close'
      ...
      >
        <div className='relative flex w-full mx-auto items-center'>
          ...
          >
            <div
              className={`flex border-r-2 border-black pr-2 ${
          ...
          <div className='absolute left-1/2 transform -translate-x-1/2 flex gap-8'>
            {links.map((link, i) => (
              <p key={i} className={isScrolled ? 'text-black' : 'text-white'}>
                {link.label}
              </p>
            ))}
          </div>
